        ...
        if (DownloadManager.ACTION_DOWNLOAD_COMPLETE.equals(action)) {
            ...
            if (c.moveToFirst()) {
                ...
                if (status == DownloadManager.STATUS_SUCCESSFUL) {
                    ...
                    String inFile = c.getString(c.getColumnIndex(DownloadManager.COLUMN_LOCAL_URI));
                    String dlTitle = c.getString(c.getColumnIndex(DownloadManager.COLUMN_TITLE));
                    String dlId = c.getString(c.getColumnIndex(DownloadManager.COLUMN_ID));
                    c.close();
                    ...
                    if (inPath.endsWith(".mp4")) {
                       new NotificationUtils(context).showNotification(Integer.parseInt(dlId), dlTitle, "Download successful", "Downloaded " + dlTitle+".mp4", false, null);
                    } else if (inPath.endsWith(".m4a")) {
                         new NotificationUtils(context).showNotification(Integer.parseInt(dlId), dlTitle, "Download successful", "Downloaded " + dlTitle+".m4a", false, null);
                    }
